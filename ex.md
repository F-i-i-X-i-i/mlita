...
Пусть дана $F$ в которую входит $X: F(...X...)$, и $P$ – тоже формула. Тогда запись $F(...P...)\  \{P//X\}$ обозначают формулу, полученную из $F$ **подстановкой** $P$ вместо всех вхождений $X$ в формулу $F$, а $F(...P...)\ \{P/X\}$ – **заменой** некоторого вхождения $X$ на формулу $P$.
...
Формальная теория $\Tau$ называется **семантически непротиворечивой**, если никакая *теорема* теории $\Tau$ не является противоречием.

> Определение: если в выводе отсутствуют гипотезы, то $G$ выводима из аксиом теории $Т$. Тогда $G$ является **теоремой** теории $Т$.
...
$n$-**местным предикатом** $P(x_1,...,x_n)$ на множестве $M$ называют функцию от $n$ аргументов $x_1,...,x_n$, областью определения которой является $M$, а областью значений — множество $\{T,F\}$.
...
$$\vdash_K \forall x A(x) \Rightarrow \vdash_K \forall y A(y)\\
\vdash_K \exists x A(x) \Rightarrow \vdash_K \exists y A(y)$$
...
1. $\vdash\forall x C(x) \to D \Leftrightarrow \exists y (C(y) \to D)$,
2. $\vdash\exists x C(x) \to D)\Leftrightarrow \forall y (C(y) \to D)$,
3. $\vdash D \to \forall x C(x) \Leftrightarrow \forall y (D \to C(y))$,
4. $\vdash D \to \exists x C(x) \Leftrightarrow \exists y (D \to C(y))$
...
3. В ходе выполнения резольвирования в п.2 возможны следующие варианты:
    - Среди дизъюнктов множества $\Gamma$ **нет резольвируемых**. Т.о., существует интерпретация, в которой все формулы $\Gamma$ истинны, $\Rightarrow G$ не является тождеством, тогда $(F_1,..,F_n)\not\Rightarrow G$ и говорят, что $\Gamma$ - выполнимо, теорема опровергнута;
...
1. В процессе резолюций с учётом унификации получен пустой дизъюнкт. Тогда множество $\Gamma$ **противоречиво** (не имеет модели), и формула $G$ логически следует из $(F_1, F_2, ..., F_n)$ - **теорема доказана**,
2. В процессе резолюций больше не удаётся получить новых резольвент. Тогда существует (эрбрановская) модель множества $\Gamma$, в которой все формулы из $\Gamma$, включая $\neg G$, истинны. Следовательно, $G$ не является логическим следствием - **теорема
опровергнута**,
...
Машина работает с неотрицательными целыми числами. Числа представляются в виде $0-1,\ 1-11,\ 2 - 111,..,\ k-1^{k+1}$
...
Функция $\varphi(n) = A$ называется **нумерацией алгоритма**, где $n$ - номер алгоритма $A$ в нумерации $\varphi$.
...
(2) Пусть C - класс задач, класс вычислим. функций одной переменной. Класс С нетривиален в том смысле что существуют функции как $\in C$, так и $\notin C$.
...
2. задача построения гамильтонова цикла во взвешенном неографе с наименьшим
суммарным весом ребер (задача коммивояжёра) и другие.
...
Задачи, где экспоненциальная сложность заложена в формулировке
(например, поиск всех подмножеств множества).
...
Задача считается решённой, если существует хотя бы одна вычислительная ветвь,
приводящая к принятию.


![](/img/35_1.png)

Множество всех задач, разрешимых на НМТ за полиномиальное время, образует **класс NP** (non-
...
3) время проверки решения $Z$ есть $t_\text{пр}=h(N)$, где $h$ – полиномиальная функция.
